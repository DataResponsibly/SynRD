...
from SynRD.publication import Finding, Publication, TAXONOMY
...
class Fruiht2018Naturally(Publication):
        ...
        self.FINDINGS = self.FINDINGS + [
            ...
            Finding(
                ...
                text="""The overall model was significant, F (8,
                            ...
                            Table 2).""",
                finding_type=TAXONOMY.REGRESSION.value.PATH_ANALYSIS.value.VARIABILITY,
            ),
            Finding(
                self.finding_390_2,
                description="finding_390_2",
    ...
    def _transform_separate_dataframes(self, wave1, wave3, wave4, print_debug=False):
        ...
        wave1["RACE_OTHER"] = (
            (wave1["RACE"] == 3) | (wave1["RACE"] == 4) | (wave1["RACE"] == 5)
        )
        ...
        wave4["EDU_ATTAINED_BINARY"] = wave4.apply(
            transform_participant_graduated, axis=1
        )

        return wave1, wave3, wave4

    def _transform_merge_dataframes(self, wave1, wave3, wave4, print_debug=False):
        merged_wave_13 = wave1.merge(
            wave3, how="inner", on="AID", validate="one_to_one"
        )
        merged_wave_all = merged_wave_13.merge(
            wave4, how="inner", on="AID", validate="one_to_one"
        )
        ...
        if print_debug:
            ...
            print(
                "Before (merged wave 1 & 3), number of rows:", merged_wave_13.shape[0]
            )

            print(
                "After (merged wave 1, 3 & 4), number of rows:",
                merged_wave_all.shape[0],
            )
            print(
                "Percentage reduction:",
                (merged_wave_13.shape[0] - merged_wave_all.shape[0])
                / merged_wave_13.shape[0],
            )
            print(
                "Percentage reduction (including reported 28 missing values):",
    ...
    def _transform_clean_merged_dataframe(self, merged_wave_all, print_debug=False):
        ...
        if print_debug:
            print(
                "Number of participants after cleaning:", merged_wave_all_clean.shape[0]
            )
            print(
                "Number of dropped participants due to missing / non-response:",
    ...
    def _recreate_dataframe(
        self, filename="fruiht2018naturally_dataframe.pickle", print_debug=False
    ):
